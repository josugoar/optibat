...
De esta forma, se pretende describir el origen de estas fuentes de datos, las reglas que gobiernan su publicación y acceso, y la arquitectura implementada para consumirlos de manera programática\footnote{El desarrollo del entorno de mercado se trata de una solución \textit{ad-hoc}, necesaria para cumplir con las restricciones corporativas de la entidad energética, a diferencia de la calidad profesional superior del resto de componentes.}. Se desarrolla en colaboración con los agentes de mercado de la entidad energética correspondiente.
...
Esto se debe a que el funcionamiento del mercado eléctrico permite negociar la energía de un mismo periodo en múltiples sesiones, por lo que hay que llevar la cuenta de la cantidad de energía anteriormente ofertada.
...
Para cumplir con esta misión crítica, el resultado puramente económico de la casación del mercado gestionado por \gls{omie} debe ser supervisado y, en ocasiones, modificado por \gls{ree}. Esto se debe a que un programa de producción económicamente óptimo podría no ser físicamente viable debido a las limitaciones de la red. Por esta razón, \gls{ree} emite consignas operativas de cumplimiento necesario que se aplican sobre el resultado del mercado, conocidas como limitaciones por seguridad.

\subsection{Limitaciones por seguridad}%
\label{makereference4.2.1}

Las limitaciones por seguridad son limitaciones de potencia impuestas por \gls{ree} a los participantes del mercado para resolver congestiones en la red o garantizar la seguridad del suministro en tiempo real. Estas limitaciones representan un límite superior inviolable a la operación de los activos energéticos y, por tanto, constituyen una entrada fundamental para el sistema de optimización.
...
Es importante tener en cuenta que, si bien las restricciones de las limitaciones por seguridad son altamente críticas, existen medios externos para garantizar que no se sobrepasan si ocurre algún error en la operación del sistema desarrollado, por lo que la estabilidad de la red nunca podrá verse afectada por un simple incumplimiento de las consignas por parte del sistema.

Con esto, la información de estas limitaciones se publica ``3 horas posteriores al periodo actual hasta el final del día actual (D), o del día siguiente (D+1)''~\cite{mte2019resolucion}. Como entre sesiones de mercado apenas existe una diferencia de una hora, no es estrictamente necesario consultar las limitaciones constantemente, sino tan solo antes de realizar la optimización.

Las limitaciones, visualizadas en la figura~\ref{fig:limitaciones-unidad}, son desglosadas según diferentes identificadores.

\begin{itemize}

  \item Una limitación a nivel de \gls{ufi} de la instalación aplica un límite de exportación de potencia a un activo de generación o almacenamiento específico e individual. Por ejemplo, podría limitar la producción de una planta fotovoltaica concreta dentro de un complejo más grande.
...
\begin{figure}
  \centering
  \includegraphics[width=0.5\linewidth]{figures/limitaciones-unidad.png}
  \caption[Visualización del desglose de las limitaciones.]{Visualización del desglose de las limitaciones, en donde se obtiene una limitación por cada telemedida, más allá del \gls{pf}~\cite{cnmc2025procedimientos}.}%
  \label{fig:limitaciones-unidad}
\end{figure}

Por tanto, el sistema de optimización desarrollado debe consultar la existencia de estas limitaciones por seguridad, de la misma forma que la información anterior. Si se recibe una limitación, el modelo debe considerarla como una restricción máxima en sus cálculos, ajustando la consigna de operación de la batería para asegurar que la exportación total de las unidades se mantenga siempre por debajo del límite impuesto por \gls{ree}. La información de limitaciones por seguridad se conoce como \gls{limitacionsuj}.

\section{Situación meteorológica}%
\label{makereference4.3}

Además de las señales económicas de \gls{omie} y las operativas de \gls{ree}, existe una tercera fuente de datos externos que resulta indispensable para el sistema de optimización, concretamente en el caso de las topologías híbridas. Esta se trata de la previsión de la generación eléctrica que producen los activos de generación (generalmente renovable) asociados a los sistemas de almacenamiento, dependientes de la situación meteorológica.
...
Se han desarrollado dos programas principales en Python, uno para cada fuente de datos, que se ejecutan de forma periódica según los horarios de publicación de los ficheros de cada mercado con respecto al cierre de los mismos, si es aplicable, representado en la figura~\ref{fig:horarios-publicacion}.
...
La adquisición de datos de \gls{omie}, aunque dispone de una interfaz web, no es apta para una integración automatizada. Por ello, la comunicación se realiza a través del servidor FTP que \gls{omie} proporciona a los agentes de mercado registrados, ya que la \gls{api} que la web pública~\cite{omie2025acceso} usa no ofrece los datos en tiempo real debido a un periodo de confidencialidad\footnote{\Gls{omie} también proporciona el cliente y aplicación de escritorio SIOM capaz de realizar descargas automáticas, pero está orientada al uso local individual de los agentes de mercado.}.

El programa utiliza la biblioteca Paramiko\footnote{\url{https://github.com/paramiko/paramiko} (LGPL-2.1)} para establecer una conexión segura (siendo necesario ser agente de mercado) mediante FTP\footnote{\url{ftp://ftp.omip.pt:8021}}. Una vez conectado, el programa implementa una lógica para asegurar que siempre se obtiene la versión más reciente de cada fichero.
...
Para obtener las limitaciones por seguridad de \gls{ree}, el enfoque es similar. Se debe utilizar la \gls{api} privada de ESIOS\footnote{\url{https://participa.esios.ree.es/ServicioSM/ServiceEME}}, exclusiva para agentes de mercado~\cite{red2025esios}, pues la \gls{api} pública tan solo proporciona la información tras un periodo de confidencialidad. La comunicación se realiza mediante SOAP a través \textit{web services} simplificados usando la biblioteca Requests\footnote{\url{https://github.com/psf/requests} (Apache-2.0)}, autenticación PKI con un certificado firmado por la autoridad certificadora OmieCA\@ e introduciendo el xsd correspondiente, basado en el código de ejemplo proporcionado por \gls{ree}\footnote{\url{https://bitbucket.org/smree/workspace/projects/EEMWS} (GPL-3.0)}.
...
Aun con todo, existe un enorme problema que impide el uso de los resultados de la energía casada de forma directa. Esto se debe a que el sistema desarrollado necesita la información de la energía casada no por \gls{up}, sino por \gls{ufi}, de tal forma que se pueda arbitrar de forma independiente.

Para resolver este problema, se hace uso de una herramienta propietaria propia de la entidad energética pertinente, que es capaz de calcular los desgloses de la energía, convirtiendo la energía casada por \gls{up} a energía casada por \gls{ufi}. Para llevarlo a cabo, debe hacer uso de información no disponible por el desarrollo, por lo que la existencia de una herramienta del estilo es necesaria.

Junto a ello, la información de limitaciones por seguridad\footnote{La información de limitaciones por seguridad es confidencial, en cambio, la información pública agregada puede ser consultada en el fichero \gls{i90dia} publicado por \gls{ree}.} se estructura en mensajes XML con las respectivas \glspl{up} y \glspl{ufi} e intervalos temporales minutales durante los cuales está vigente la limitación, necesitando transformarlos a periodos de mercado cuartohorarios, según el diagrama de la figura~\ref{fig:contenido-limitaciones} y la siguiente ejemplificación del contenido.

\begin{figure}
  \centering
  \includegraphics[height=0.5\textheight]{figures/contenido-limitaciones.png}
...
\begin{samepage}
  \begin{minted}[fontsize=\scriptsize, breaklines]{xml}
    <?xml version="1.0"?>
    <LimitacionesSujeto xmlns="urn:sios.ree.es:limitacionessujeto:1:0">
      <IdentificacionMensaje v="limitacionessm_20251001"/>
      <VersionMensaje v="6"/>
      <TipoMensaje v="Z20"/>
      <TipoProceso v="Z13"/>
      ...
      <IdentificacionDestinatario codificacion="A01" v="DESTINATARIO"/>
      <FuncionDestinatario v="A08"/>
      <FechaHoraMensaje v="2025-09-30T16:27:59Z"/>
      <Horizonte v="2025-09-30T16:00Z/2025-10-01T22:00Z"/>
      <SeriesTemporales>
        <IdentificacionSeriesTemporales v="STP1"/>
        <TipoNegocio v="Z10"/>
        <UP codificacion="NES" v="FBHIBHV"/>
        <UF codificacion="NES" v="PLLANO" />
        <UnidadMedida v="MAW"/>
        <Periodo>
          <IntervaloTiempo v="2025-09-30T16:00Z/2025-09-30T17:00Z"/>
          <Intervalo>
            <TipoCategoriaRestriccion v="Z05"/>
            <Descripcion v="Programa 24.5"/>
            <SubIntervalo>
              <TipoLimitacion v="Z02"/>
              <PotenciaLimite v="24.5"/>
            </SubIntervalo>
          </Intervalo>
        </Periodo>
      </SeriesTemporales>
...
Todos estos archivos son transformados al formato interno de inserción y entregados al intercambiador de ficheros, para ser insertados directamente en base de datos.
